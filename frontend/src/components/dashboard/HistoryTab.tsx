  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto py-2 text-left">
      <div className="space-y-1">
        <h2 className="text-xl font-semibold text-slate-800">Scan Audit History</h2>
        <p className="text-sm text-slate-500">Track and restore previous resume audits and scoring evaluations.</p>
      </div>

      {/* Active Scan Indicator Card */}
      {activeAnalysis && (
        <div className="border border-blue-100 bg-blue-50/20 p-5 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-sm">
          <div className="flex items-center space-x-3.5">
            <div className="w-10 h-10 rounded-xl bg-blue-500 text-white flex items-center justify-center shrink-0">
              <Award className="w-5.5 h-5.5" />
            </div>
            <div>
              <span className="text-[10px] font-bold text-blue-600 uppercase tracking-wider block">Latest Session Scan</span>
              <h4 className="text-sm font-bold text-slate-800 leading-snug">{activeAnalysis.jd_title}</h4>
              <p className="text-[11px] text-slate-400">ATS Score: {activeAnalysis.ats_score}% · Match Alignment: {activeAnalysis.match_score}%</p>
            </div>
          </div>
          <button
      ...
      <div className="border border-slate-100 bg-white rounded-2xl shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-50 bg-slate-50/50 flex items-center space-x-2">
          ...
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Historical Scans</h3>
        </div>
              ...
              <div key={item.id} className="p-4 flex flex-col sm:flex-row justify-between sm:items-center gap-4 hover:bg-slate-50/30 transition-colors">
                <div className="flex items-start space-x-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 truncate">{item.company} · Scanned on {item.date}</p>
                    <div className="flex items-center space-x-3 mt-1">
                      <span className="text-[9px] font-semibold px-2 py-0.2 bg-emerald-50 text-emerald-600 rounded border border-emerald-100">
                        Match: {item.matchScore}%
                      </span>
                      <span className="text-[9px] font-semibold px-2 py-0.2 bg-blue-50 text-blue-600 rounded border border-blue-100">
                        ATS: {item.atsScore}%
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex items-center space-x-2.5 shrink-0 self-end sm:self-auto">
                  <button
                    ...
                    className="flex items-center space-x-1 px-3 py-1.5 border border-slate-200 hover:bg-slate-100 rounded-lg text-[10px] font-bold text-slate-600 transition-all"
                  >
